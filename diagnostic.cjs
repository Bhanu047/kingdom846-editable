...
function snippets(text, needle, radius=180){
  ...
  while((p=low.indexOf(n,p))>=0 && out.length<60){ out.push(text.slice(Math.max(0,p-radius), Math.min(text.length,p+n.length+radius))); p+=n.length }
  return out
}
async function inspect(which){
  ...
  const result={pageUrl,status:r.status,htmlLength:html.length,scriptSrcs,inlineCount:inline.length,html846:snippets(html,'846'),inline846:inline.flatMap(s=>snippets(s,'846')).slice(0,60),scriptFindings:[]}
  for(const src of scriptSrcs.slice(0,30)){
    try{
      ...
      const hitsApi=[...snippets(txt,'/api/',220),...snippets(txt,'fetch(',220),...snippets(txt,'supabase',220),...snippets(txt,'graphql',220),...snippets(txt,'kingdom',140)].slice(0,80)
      if(hits846.length||hitsApi.length||urls.length) result.scriptFindings.push({src,status:sr.status,length:txt.length,urls:urls.slice(0,80),hits846:hits846.slice(0,25),hitsApi:hitsApi.slice(0,40)})
...
server.listen(process.env.PORT||10000,()=>console.log('diagnostic listening'))
