(() => {
  ...
  function readModalValues(modal) {
    const values = { capacity: null, infantry: {}, cavalry: {}, archers: {}, troopCounts: null }
    modal.querySelectorAll('[data-field]').forEach((input) => {
      const value = clean(input.value)
      if (value == null) return
      if (input.dataset.field === 'capacity') { values.capacity = value; return }
    ...
    if (side && raw) {
      for (const troop of TYPES) {
        for (const stat of STATS) {
          ...
          if (detected.length >= 2) values[troop.key][stat.key] = side === 'right' ? detected[detected.length - 1] : detected[0]
        }
      }
    }
    return values
  ...
  function troopCard(section, troop) {
    ...
    candidates.sort((a, b) => a.querySelectorAll('input[type="number"]').length - b.querySelectorAll('input[type="number"]').length || (a.textContent || '').length - (b.textContent || '').length)
  ...
  function saveToLocalProfile(values) {
    let profiles = []
    try { const parsed = JSON.parse(localStorage.getItem(STORAGE_KEY) || '[]'); profiles = Array.isArray(parsed) ? parsed : [] } catch {}
    ...
    const profile = {
      ...
      name: existing?.name || 'My Player', kingdom: existing?.kingdom || '846',
      ...
      sources: existing?.sources || {}, updatedAt: new Date().toISOString(),
    }
    for (const troop of TYPES) for (const stat of STATS) {
    ...
    window.dispatchEvent(new CustomEvent('k846:report-applied', { detail: report }))
    return profile
  }

  function toast(message, tone = 'ok') {
    const node = document.createElement('div'); node.textContent = message
    Object.assign(node.style, { position:'fixed',left:'50%',bottom:'26px',zIndex:10070,transform:'translateX(-50%)',maxWidth:'92vw',padding:'11px 15px',borderRadius:'10px',font:'700 11px Montserrat, sans-serif',color:tone==='error'?'#ffd5d5':'#f6e5ad',background:tone==='error'?'rgba(91,18,24,.98)':'rgba(10,24,43,.98)',border:`1px solid ${tone==='error'?'rgba(255,120,120,.3)':'rgba(212,175,55,.34)'}`,boxShadow:'0 14px 40px rgba(0,0,0,.45)' })
