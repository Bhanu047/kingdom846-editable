(() => {
  ...
  // Matches TROOP_COLORS in src/pages/MysticSuite.jsx (infantry gold, cavalry
  // blue, archers red) -- this file used a different mapping (infantry blue,
  // cavalry gold) that disagreed with every other troop-color chart on the site.
  const COLORS = { infantry: '#d9b94e', cavalry: '#7f9ed6', archers: '#c8655a' }
  ...
  function render() {
    ...
    root.innerHTML = `
      ...
      <div class="k846-fa-card">
        ...
        <div class="k846-fa-stack"><span style="width:${pi}%;background:linear-gradient(180deg, rgba(255,255,255,.35), ${COLORS.infantry});box-shadow:inset 0 0 8px ${COLORS.infantry}66"></span><span style="width:${pc}%;background:linear-gradient(180deg, rgba(255,255,255,.35), ${COLORS.cavalry});box-shadow:inset 0 0 8px ${COLORS.cavalry}66"></span><span style="width:${pa}%;background:linear-gradient(180deg, rgba(255,255,255,.35), ${COLORS.archers});box-shadow:inset 0 0 8px ${COLORS.archers}66"></span></div>
      ...
      <div class="k846-fa-grid">
        <div class="k846-fa-card">
          ...
          <div style="position:relative">
            <svg class="k846-fa-svg" viewBox="0 0 ${W} ${H}" aria-label="Formation response curve">
              ...
              <path d="${paths.infantry}" fill="none" stroke="${COLORS.infantry}" stroke-width="3" style="filter:drop-shadow(0 0 4px ${COLORS.infantry}99)"/>
              <path d="${paths.cavalry}" fill="none" stroke="${COLORS.cavalry}" stroke-width="3" style="filter:drop-shadow(0 0 4px ${COLORS.cavalry}99)"/>
              <path d="${paths.archers}" fill="none" stroke="${COLORS.archers}" stroke-width="3" style="filter:drop-shadow(0 0 4px ${COLORS.archers}99)"/>
            </svg>
            <!-- Plain HTML labels, not SVG <text> -- html2canvas 1.4.1 renders
                 SVG text (and the whole SVG along with it) blank in exported
                 reports, and this chart is cloned straight into the
                 Formation Report PNG by k846-official-reports-v2.js. -->
            ${[-20,-10,0,10,20].map(d=>`<div style="position:absolute;left:${(x(d)/W*100).toFixed(2)}%;bottom:1%;transform:translateX(-50%);font-size:10px;color:rgba(244,236,211,.46)">${d>0?'+':''}${d}%</div>`).join('')}
            <div style="position:absolute;left:${(x(0)/W*100).toFixed(2)}%;top:1%;transform:translateX(-50%);font-size:10px;font-weight:700;color:#f5d778">OPTIMAL</div>
          </div>
        ...
        <div class="k846-fa-card">
          ...
          <div class="k846-fa-strength">
            ${TYPES.map(k=>`<div class="k846-fa-strength-row"><b style="color:${COLORS[k]}">${k==='infantry'?'INF':k==='cavalry'?'CAV':'ARC'}</b><div class="k846-fa-track"><div class="k846-fa-fill" style="width:${strength[k].toFixed(1)}%;background:linear-gradient(90deg, ${COLORS[k]}77, ${COLORS[k]});box-shadow:0 0 6px ${COLORS[k]}aa"></div></div><span style="color:${COLORS[k]};font-family:JetBrains Mono,monospace">${strength[k].toFixed(0)}</span></div>`).join('')}
          </div>
