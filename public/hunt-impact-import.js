(() => {
  ...
  function style(){if(document.getElementById('hi-style-v2'))return;const s=document.createElement('style');s.id='hi-style-v2';s.textContent=`#${MODAL_ID}{position:fixed;inset:0;z-index:10060;height:100vh;height:100dvh;height:var(--k846-vvh,100dvh);display:grid;place-items:center;place-items:safe center;overflow-y:auto;padding:10px;padding-bottom:calc(10px + env(safe-area-inset-bottom));font-family:Montserrat,system-ui}#${MODAL_ID} .hi-backdrop{position:absolute;inset:0;background:rgba(2,8,20,.9)}#${MODAL_ID} .hi-dialog{position:relative;width:min(760px,100%);max-height:94vh;max-height:94dvh;max-height:var(--k846-modal-max,94dvh);overflow:hidden;display:flex;flex-direction:column;border:1px solid #6d5a24;border-radius:20px;background:#08172a;color:#f1e7ce}#${MODAL_ID} header{flex:0 0 auto;display:flex;justify-content:space-between;padding:18px;border-bottom:1px solid #3a321c}#${MODAL_ID} header button{background:none;border:0;color:#fff;font-size:28px}#${MODAL_ID} .eye,#${MODAL_ID} em{color:#d4af37;font-style:normal;font-size:10px}#${MODAL_ID} h2{margin:4px 0;font-family:Cinzel,serif}#${MODAL_ID} p{margin:0;color:#a9a9a9;font-size:11px}#${MODAL_ID} .body{flex:1 1 auto;min-height:0;padding:16px;overflow:auto}#${MODAL_ID} .drop{min-height:150px;display:grid;place-items:center;border:1px dashed #8d742b;border-radius:15px;cursor:pointer;text-align:center}#${MODAL_ID} .drop span{display:block;color:#777;font-size:10px}#${MODAL_ID} .preview{display:flex;gap:12px;align-items:center}#${MODAL_ID} .preview[hidden]{display:none}#${MODAL_ID} .preview img{width:90px;height:70px;object-fit:cover;border-radius:10px}#${MODAL_ID} .status{margin:10px 0;color:#c8c0a6;font-size:11px}#${MODAL_ID} .grid{display:grid;grid-template-columns:1fr 1fr;gap:10px}#${MODAL_ID} .card{display:grid;grid-template-columns:repeat(3,1fr);gap:8px;margin-top:10px;padding:12px;border:1px solid #302b1d;border-radius:12px}#${MODAL_ID} .card>b{grid-column:1/-1}#${MODAL_ID} label{display:block;font-size:10px;color:#9c978a}#${MODAL_ID} input,#${MODAL_ID} select{box-sizing:border-box;width:100%;margin-top:5px;padding:10px;border:1px solid #43391f;border-radius:9px;background:#06111f;color:#f1e7ce}#${MODAL_ID} input[readonly]{opacity:.9}#${MODAL_ID} footer{flex:0 0 auto;display:flex;justify-content:flex-end;gap:8px;padding:14px;border-top:1px solid #3a321c}#${MODAL_ID} footer button{padding:10px 14px;border-radius:10px}.apply{background:linear-gradient(#d4af37,#ad8620);color:#071224;font-weight:900}.apply:disabled{opacity:.35}@media(max-width:640px){#${MODAL_ID}{align-items:end}#${MODAL_ID} .card{grid-template-columns:1fr 1fr}#${MODAL_ID} .card label:last-child{grid-column:1/-1}}`;document.head.appendChild(s)}
