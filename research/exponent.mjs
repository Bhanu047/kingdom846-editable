...
function sim(att, def, E, maxTurns = 2000) {
  ...
  return { edge: (sA ? ra / sA : 0) - (sD ? rd / sD : 0), clears: rd === 0 && ra > 0 }
}
...
function bestFor(r, E) {
  ...
  for (let i = 0; i <= 100; i += 1) for (let c = 0; c + i <= 100; c += 1) {
    ...
    if (!best || s.edge > best.edge) best = { split: [i, c, a], edge: s.edge, clears: s.clears }
  }
  return best
}

console.log('E      KN            MF            FoL           total err')
const rows = []
for (const E of [0.5, 0.6, 0.7, 0.75, 0.8, 0.85, 0.9, 0.95, 1.0]) {
  const b = REPORTS.map((r) => bestFor(r, E))
  const d = b.map((x, i) => dist(x.split, REPORTS[i].frak))
  const tot = d.reduce((s, v) => s + v, 0)
  rows.push({ E, b, d, tot })
  console.log(` ${E.toFixed(2)}  ${b.map((x, i) => (x.split.join('/') + ' (' + d[i] + ')').padEnd(14)).join('')}${tot}`)
}
const win = [...rows].sort((a, b) => a.tot - b.tot)[0]
console.log(`\nBEST E = ${win.E}  total error ${win.tot}   (current shipped E = 0.5 -> ${rows[0].tot})`)
