...
export default function SplashScreen({ onEnter }) {
  ...
  return (
    <div className={`fixed inset-0 z-[100] flex items-center justify-center overflow-hidden ${fading ? 'splash-fade-out' : ''}`} style={{ background: '#060810' }}>
      ...
      <div className="absolute inset-0 overflow-hidden" style={{ zIndex: 4, pointerEvents: 'none' }}>
        {Array.from({ length: particleCount }).map((_, i) => (
          <span key={i} style={{ position: 'absolute', bottom: '-10px', left: `${(i * 17) % 100}%`, width: `${2 + (i % 3)}px`, height: `${2 + (i % 3)}px`, borderRadius: '50%', background: i % 2 ? '#E8C766' : '#D4AF37', boxShadow: '0 0 8px rgba(212,175,55,.55)', opacity: 0, animation: `ember-rise ${8 + (i % 5)}s linear ${(i * .35) % 8}s infinite` }} />
        ))}
      </div>
      ...
      <main className="relative mx-auto flex w-full max-w-3xl flex-col items-center px-6 text-center" style={{ zIndex: 10 }}>
        <div className="relative flex items-center justify-center">
          ...
          <img src="./assets/crest-846.png" alt="Kingdom 846 Royal Crest" loading="eager" className="relative w-auto object-contain drop-shadow-2xl" style={{ maxHeight: isMobile ? '29vh' : '38vh', maxWidth: '76vw' }} />
        </div>
