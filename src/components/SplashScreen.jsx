  ...
  return (
    <div className={`premium-welcome fixed inset-0 z-[100] flex items-center justify-center overflow-hidden ${fading ? 'portal-opening' : ''}`}>
      ...
      <style>{`
        ...
        .welcome-backdrop { position:absolute; inset:-6%; z-index:-3; background-image:url('/assets/kingdom846-welcome-premium.webp'); background-position:center; background-size:cover; filter:blur(30px) brightness(.32) saturate(1.16); transform:scale(1.08); }
        ...
        .welcome-art-frame picture,.welcome-art { display:block; width:100%; height:100%; }
        .welcome-art { object-fit:cover; user-select:none; -webkit-user-drag:none; }
        ...
        @media (max-width:767px) { .welcome-backdrop { background-image:url('/assets/kingdom846-welcome-mobile.webp'); filter:blur(24px) brightness(.34) saturate(1.12); } .welcome-vignette { background:linear-gradient(180deg,rgba(2,3,7,.5),transparent 11%,transparent 89%,rgba(2,3,7,.58)); } .welcome-art-frame { width:130vw; max-width:none; aspect-ratio:2/3; box-shadow:0 0 55px rgba(0,0,0,.65); } .welcome-enter-hit { left:26.5%; top:73.1%; width:47%; height:8.2%; border-radius:12px; } .welcome-sheen { display:none; } }
        @media (max-height:650px) and (max-width:767px) { .welcome-art-frame { width:112vw; } }
