  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-x-0 top-0 flex items-start justify-between p-5 md:p-6">
        <div className="grid h-12 w-12 place-items-center rounded-2xl border border-gold/35 bg-black/35 text-gold-bright shadow-lg backdrop-blur"><Icon name={tool.icon} size={23} /></div>
        <BetaBadge />
      </div>
      <div className="absolute inset-x-0 bottom-0 p-5 md:p-7">
        ...
        <p className="mt-2 max-w-md text-sm leading-relaxed text-parchment/75 md:text-[15px]">{tool.description}</p>
  ...
  if (!activeTool) {
    return (
      <div className="space-y-6">
        <section className="relative overflow-hidden rounded-[26px] border border-gold/25 bg-[#06101f] px-6 py-9 text-center shadow-[0_20px_70px_rgba(0,0,0,.32)] md:px-10 md:py-12">
          <div className="pointer-events-none absolute inset-0 opacity-55" style={{ background: 'radial-gradient(circle at 50% 0%, rgba(212,175,55,.18), transparent 44%), linear-gradient(135deg, rgba(212,175,55,.06), transparent 35%)' }} />
          <div className="relative">
            <div className="mx-auto mb-4 grid h-14 w-14 place-items-center rounded-2xl border border-gold/30 bg-gold/10 text-gold-bright"><Icon name="crosshair" size={27} /></div>
            <h1 className="font-display text-4xl font-bold uppercase tracking-[0.05em] text-parchment md:text-5xl">Battle Lab</h1>
            <p className="mx-auto mt-3 max-w-2xl text-sm text-parchment/55 md:text-base">Four tactical tools. Choose your mission.</p>
