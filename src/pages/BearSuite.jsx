    ...
    <div className="space-y-5">
      <section className="panel p-4 md:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div><div className="eyebrow">Bear Calculator</div><h2 className="mt-1 font-display text-2xl font-bold text-parchment">Ratio + Damage</h2><p className="mt-1 max-w-2xl text-xs leading-relaxed text-parchment/50">Enter Rally Bonus Attack and Lethality from the setup you actually use for Bear. The two tools share the same stats.</p></div>
          <div className="inline-flex rounded-xl border border-gold/15 bg-black/25 p-1">
            <button type="button" onClick={() => setTab('ratio')} className={`rounded-lg px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] transition ${tab === 'ratio' ? 'bg-gold/15 text-gold-bright' : 'text-parchment/45 hover:text-parchment'}`}>Bear Ratio</button>
            <button type="button" onClick={() => setTab('damage')} className={`rounded-lg px-4 py-2 text-xs font-bold uppercase tracking-[0.12em] transition ${tab === 'damage' ? 'bg-gold/15 text-gold-bright' : 'text-parchment/45 hover:text-parchment'}`}>Bear Damage</button>
          </div>
        </div>
      </section>
      ...
      <div className="grid gap-5 xl:grid-cols-[.82fr_1.18fr]">
        <section className="panel p-5 md:p-6">
          ...
          <div className="mt-4 rounded-xl border border-sky-300/15 bg-sky-300/[0.035] p-3 text-[11px] leading-relaxed text-sky-100/65">Use the troop Attack and Lethality shown in your Rally Bonus screen. Defense, Health and Squad stats are not inputs to this Bear ratio/damage model.</div>
        </section>

        {tab === 'ratio' ? (
          <section className="panel panel-glow p-5 md:p-6">
            <div className="flex items-center justify-between gap-3"><div><div className="eyebrow">Bear Ratio</div><h3 className="mt-1 font-display text-2xl font-bold text-parchment">Optimal Troop Split</h3></div><div className="font-mono text-sm font-bold text-gold-bright">{Number(capacity).toLocaleString()}</div></div>
            ...
            <div className="mt-4 grid grid-cols-2 gap-3"><div className="rounded-xl border border-gold/10 bg-white/[0.025] p-3"><div className="text-[9px] uppercase tracking-wider text-parchment/35">Damage gain</div><div className="mt-1 font-mono text-xl font-bold text-gold-bright">+{result.gainVsBalanced.toFixed(2)}%</div><div className="text-[10px] text-parchment/35">vs 33/33/33</div></div><div className="rounded-xl border border-gold/10 bg-white/[0.025] p-3"><div className="text-[9px] uppercase tracking-wider text-parchment/35">Archer modifier</div><div className="mt-1 font-mono text-xl font-bold text-gold-bright">×{result.arcMult.toFixed(2)}</div><div className="text-[10px] text-parchment/35">tier/TG rule</div></div></div>
            <button type="button" onClick={copyFormation} className="btn-primary btn-royal mt-4 w-full justify-center"><Icon name={copied ? 'shieldCheck' : 'scroll'} size={14} /> {copied ? 'Copied' : 'Copy Bear ratio'}</button>
          ...
          <section className="panel panel-glow p-5 md:p-6">
            <div className="eyebrow">Bear Damage</div><h3 className="mt-1 font-display text-2xl font-bold text-parchment">Damage Comparison</h3><p className="mt-1 text-xs text-parchment/45">Compare any troop ratio against the calculated optimum and the equal 33/33/33 split.</p>
            ...
            <div className="mt-5 grid gap-3 sm:grid-cols-3">
              <div className="rounded-2xl border border-gold/15 bg-gold/[0.045] p-4"><div className="text-[9px] font-bold uppercase tracking-wider text-gold/55">Your score</div><div className="mt-1 font-mono text-2xl font-bold text-gold-bright">{customScore.toFixed(3)}</div><div className="mt-1 text-[10px] text-parchment/35">relative damage</div></div>
            ...
            <div className="mt-5 rounded-2xl border border-gold/15 bg-black/20 p-4"><div className="flex items-center justify-between"><span className="text-xs font-semibold text-parchment/60">Rally damage index</span><span className="font-mono text-lg font-bold text-gold-bright">{Math.round(rallyDamageIndex).toLocaleString()}</span></div><div className="mt-2 grid grid-cols-3 gap-2 text-center"><div><div className="text-[9px] uppercase text-parchment/30">Participants</div><div className="mt-1 text-sm font-bold text-parchment">{participants}</div></div><div><div className="text-[9px] uppercase text-parchment/30">Per player</div><div className="mt-1 text-sm font-bold text-parchment">{perParticipant.toLocaleString()}</div></div><div><div className="text-[9px] uppercase text-parchment/30">Fill</div><div className="mt-1 text-sm font-bold text-parchment">{fillRate.toFixed(2)}%</div></div></div></div>
            <div className="mt-4 rounded-xl border border-amber-300/15 bg-amber-300/[0.035] p-3 text-[11px] leading-relaxed text-amber-100/60">Damage score is a relative comparison index, not a prediction of the exact damage number shown by Kingshot. Use it to compare ratios with the same stats and rally setup.</div>
          </section>
        )}
      </div>
    </div>
